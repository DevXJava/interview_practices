...
public class CollectorsTest {
	...
	public static void main(String[] args) {
		...
		List<Student> top3Student = studentList.stream().sorted(Comparator.comparingDouble(Student::getPercentage).reversed()).limit(3).collect(Collectors.toList());
	 ...
	 //10. summingInt(), summingLong(), summingDouble()
	  ...
	  DoubleSummaryStatistics averagePercentage = studentList.stream().collect(Collectors.summarizingDouble(Student::getPercentage));
	  
	  //12.  Collectors.groupingBy() :
	  Map<String, List<Student>>  studentGroupBySubject = studentList.stream().collect(Collectors.groupingBy(Student::getSubject));
	  
	  //13. Collectors.partitioningBy() :  Partitioning the students who got above 80.0% from who don’t.
